 ...
 Scenario: Claims priced with: hourly rates Scheme
  ...
  Scenario: Claims priced with: Additional Payments though not entitiled
    When the following outcomes are bulkloaded:
      ...
      | 1 | 010413/001 | CM         |      01/04/2013 |          31/03/2023 |        5000 |         4000 | N             |         1 |              1 | Y                   |                     1 |
    When user confirms the submission
    And user is on the pricing outcome details page
    Then user should see the following outcomes:
      | # | UFN        | Value      | Comment                                                                                                                                                                 |
      | 1 | 010413/001 | £ 9,000.00 | Priced at hourly rates Profit_Cost(£5000) + Counsel_Cost(£4000) = £9000, ignoring additional payments as code is not entitiled for additional payments CMRH_ORAL(£166) + CMRH_TELEPHONE(£90) + SUBSTANTIVE_HEARING(£302) +  ADJOURNED_HEARING(£161) = null |
