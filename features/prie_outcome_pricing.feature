Feature: PRIE outcome pricing
  ...
  Background: User bulk loads a file with PRIE outcomes
    Given user bulk loads monthly statement with "CRIME LOWER" - "reconsideration hearing PRIE" outcomes
    And user is on the "CRIME LOWER" pricing outcome details page

  Scenario Outline: PRIE outcome pricing
    When user is looking at outcome "<UFN>"
    Then user should see the outcome with stage reached "PRIE"
