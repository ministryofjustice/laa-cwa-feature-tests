Feature: Validation for Immigration claims
  ...
  Scenario Outline: Add valid Immigration claims
    When user adds outcomes for "Legal Help" "Immigration And Asylum" with fields like this:
      | case_id | matter_type | exemption_criteria_satisfied | excl_case_funding_ref | case_start_date | procurement_area | access_point | irc_surgery |
      ...
      |     719 | IACE:IOUT   | TR001                        |                       |        01/04/23 | PA00187          | AP00187      |             |
      |     720 | IACE:IOUT   | TR001                        |                       |        01/04/23 | PA00188          | AP00186      | No          |
      |     721 | IACE:IOUT   | TR001                        |                       |        01/04/23 | PA00188          | AP00187      | No          |
    Then the outcome saves successfully

  @delete_outcome_after @manual_submission @valid @irc
  Scenario Outline: Add valid Immigration claims using IRC , the start date 18/10/21 according to guidance for PA/AP
    When user adds outcomes for "Legal Help" "Immigration" with fields like this:
      | case_id | matter_type | case_start_date | procurement_area | access_point | irc_surgery |
      |     722 | IMXL:IIRC   |        18/10/21 | PA00188          | AP00187      | No          |
    Then the outcome saves successfully
