...
Scenario: validate access point for ASYLUM where case start date >= 15/11/2010 with PA/AP code stage claims
    Given a test firm user is logged in CWA
...
Scenario: validate access point for IMMIGRATION where case start date < 15/11/2010 with PA/AP code stage claims
    Given a test firm user is logged in CWA
    And user prepares to submit outcomes for test provider "LEGAL HELP.IMMOT#20"
    Given the following Matter Types are chosen:
      | IMLB:IOUT |
    And the following outcomes are bulkloaded:
      | # | CASE_ID | UFN        | CLAIM_TYPE | CASE_START_DATE | STAGE_REACHED | PROCUREMENT_AREA | ACCESS_POINT | IRC_SURGERY |
      | 1 |     001 | 141110/001 | SC         |        14/11/10 | IR            | PA00142          | AP00187      | N           |
